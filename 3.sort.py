...
QSoybeanFM = soybeanFM_df.loc[soybeanFM_df["QTY_1_MO"] >= 0]
...
drop_list = ["STATE", "ALL_VAL_MO", "E_COMMODITY.1"]
QSoybeanFM = QSoybeanFM.drop(columns=drop_list)

drop_list = ["QTY_1_MO", "UNIT_QY1", "STATE", "E_COMMODITY.1"]
...
#Pull out grouped pieces of CSV fvariable into dataframes for
#quantity, nation value, and WI value
QSoybean = soybean_df.loc[soybean_df["QTY_1_MO"] >= 0]
...
#Rename Columns for calarity
VSoybean = VSoybean.rename(columns={'ALL_VAL_MO':'National_Value'})
WISoybean = WISoybean.rename(columns={'ALL_VAL_MO':'WI_Value'})
#%%
...
#Rename Columns for calarity
VWhey = VWhey.rename(columns={'ALL_VAL_MO':'National_Value'})
WIWhey = WIWhey.rename(columns={'ALL_VAL_MO':'WI_Value'})
#%%
...
QModWhey = QModWhey.rename(columns= {"QTY_1_MO":'ModWhey_QTY',
                                     "UNIT_QY1":"ModWhey_QTY"})
QModOtherWhey = QModOtherWhey.rename(columns= {'QTY_1_MO':'ModOtherWhey_QTY',
                                               "UNIT_QY1":"ModOtherWhey_Unit"})
QFluidOtherWhey = QFluidOtherWhey.rename(columns= {'QTY_1_MO':'FluidOtherWhey_QTY',
...
#Rename Columns for calarity
VGin = VGin.rename(columns={'ALL_VAL_MO':'National_Value'})
WIGin = WIGin.rename(columns={'ALL_VAL_MO':'WI_Value'})
#%%
#Because there are multiple quantity values that need to be aggregated
...
#Rename columns for clarity
QCultGin = QCultGin.rename(columns= {"QTY_1_MO":'CultGin_QTY',
...
Master_Gin['QTY_1_MO'] = Master_Gin[sum_list].sum(axis=1)
...
drop_list = ['QTY_1_MO','UNIT_QY1','STATE']
...
#Rename Columns for calarity
VCorn = VCorn.rename(columns={'ALL_VAL_MO':'National_Value'})
WICorn = WICorn.rename(columns={'ALL_VAL_MO':'WI_Value'})
...
#Rename Columns
QCornmealCorn = QCornmealCorn.rename(columns= {"QTY_1_MO":'Cornmeal_QTY',
                                               "UNIT_QY1":"CornmealCorn_Unit"})
QOtherCorn = QOtherCorn.rename(columns= {'QTY_1_MO':'Other_QTY',
...
Master_Corn['QTY_1_MO'] = Master_Corn[sum_list].sum(axis=1)
#%%
...
#Pull out grouped pieces of CSV fvariable into dataframes for
#quantity, nation value, and WI value
QMilk = milk_df.loc[soybeanFM_df["QTY_1_MO"] >= 0]
...
#Rename Columns for calarity
VMilk = VMilk.rename(columns={'ALL_VAL_MO':'National_Value'})
WIMilk = WIMilk.rename(columns={'ALL_VAL_MO':'WI_Value'})
#%%
